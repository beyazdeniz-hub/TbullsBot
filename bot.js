...
function getTurkeyNow() { return new Date(new Date().toLocaleString("en-US", { timeZone: "Europe/Istanbul" })); }
function formatTurkeyDateTime() {
  return new Date().toLocaleString("tr-TR", { timeZone: "Europe/Istanbul", year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", second: "2-digit" });
}
...
async function sendTelegram(text, html = false) {
  ...
  } catch (e) { console.log("Telegram mesaj hatasi:", e.response?.data || e.message); }
}
async function sendTelegramPhoto(photoBuffer, caption) {
  ...
  try {
    ...
    formData.append("photo", photoBuffer, { filename: "chart.png", contentType: "image/png" });
    await axios.post(`https://api.telegram.org/bot${TOKEN}/sendPhoto`, formData, { headers: formData.getHeaders(), timeout: 60000 });
  } catch (e) { console.log("Telegram resim hatasi:", e.response?.data || e.message); }
}
...
async function extractDetailAndChart(detailPage, ticker) {
  ...
  try {
    await detailPage.evaluate(() => {
      const banner = document.querySelector('.cc-window, .cookie-banner, [class*="cookie"], [class*="Cookie"], [id*="cookie"]');
    ...
    console.log(`${ticker} sol grafik alindi.`);
  } catch (e) { console.log(`${ticker} grafik alinamadi: ${e.message}`); }
  const levels = await detailPage.evaluate(() => {
    const bodyText = (document.body.innerText || "").replace(/\s+/g, " ");
    const pick = (patterns) => { for (const re of patterns) { const m = bodyText.match(re); if (m?.[1]) return m[1].trim(); } return "-"; };
    return {
      alSeviyesi: pick([
        /Al[^a-z]*?([0-9]+[.,][0-9]+)/i,
        /Alis[^a-z]*?([0-9]+[.,][0-9]+)/i,
...
async function uploadJsonToGithub(remotePath, data, message, retries = 2) {
  ...
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      let sha = null;
      try {
        const res = await axios.get(`https://api.github.com/repos/${GITHUB_REPOSITORY}/contents/${remotePath}?ref=${GITHUB_BRANCH}`, { headers: { Authorization: `Bearer ${GITHUB_TOKEN}` } });
        sha = res.data.sha;
      } catch (e) {}
      await axios.put(`https://api.github.com/repos/${GITHUB_REPOSITORY}/contents/${remotePath}`, { message, content, branch: GITHUB_BRANCH, ...(sha ? { sha } : {}) }, { headers: { Authorization: `Bearer ${GITHUB_TOKEN}` } });
      console.log(`GitHub yukleme basarili: ${remotePath}`);
      return;
    } catch (e) {
      if (i === retries) { console.log(`GitHub yukleme hatasi (${remotePath}):`, e.response?.data || e.message); }
      else { console.log(`GitHub yeniden deneniyor...`); await sleep(3000); }
    }
  }
}
async function run() {
  const chromePath = await resolveChromePath();
  const browser = await puppeteer.launch({ headless: true, executablePath: chromePath, args: ["--no-sandbox", "--disable-setuid-sandbox", "--window-size=1200,900"] });
  try {
    ...
    for (const ticker of testTickers) {
      try {
        ...
        if (!isNaN(alisNum) && !isNaN(stopNum) && alisNum > 0) {
          ...
          if (risk <= RISK_LIMIT) results.push({ ticker, alis: detail.alSeviyesi, stop: detail.stoploss, risk });
        }
      } catch (e) { console.log(`${ticker} islenirken hata: ${e.message}`); }
    ...
    await sendTelegram(`<b>NeuroTrade Islemi Tamamlandi</b>\nKategori: ${category}\nBulunan Sinyal: ${results.length}`, true);
