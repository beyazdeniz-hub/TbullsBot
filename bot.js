  ...
  try {
    await detailPage.evaluate(() => {
      const banner = document.querySelector(
        '.cc-window, .cookie-banner, [class*="cookie"], [class*="Cookie"], [id*="cookie"]'
      );
    ...
    console.log(`${ticker} grafik alinamadi: ${e.message}`);
  }
  ...
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      if (i === retries) {
        ...
        console.log(`GitHub yeniden deneniyor...`);
        await sleep(3000);
      }
    }
  }
...
async function run() {
  ...
  try {
    ...
    if (category === "seans")
      await uploadJsonToGithub("seans.json", payload, `Update ${updatedAt}`);

    if (category === "onay")
      await uploadJsonToGithub("onay.json", payload, `Update ${updatedAt}`);
